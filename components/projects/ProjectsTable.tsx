      ...
      <table className="w-full text-left">
        <thead>
          <tr className="bg-foreground/[0.01]">
            <th className="px-6 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-center">Order</th>
          ...
          {projects.map((project) => (
            <tr key={project.id} className="group hover:bg-foreground/[0.01] transition-colors">
              <td className="px-6 py-6 text-center">
                <span className="text-[10px] font-mono text-muted-foreground bg-foreground/5 px-2 py-1 border border-border">
                   #{project.order}
                </span>
              </td>
              <td className="px-6 py-6 min-w-[300px]">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded bg-foreground/5 border border-border flex items-center justify-center text-muted-foreground group-hover:text-accent transition-colors">
                    <project.icon size={24} />
                  </div>
                  <div>
                    <p className="text-base font-bold text-foreground group-hover:text-accent transition-colors">{project.name}</p>
              ...
              <td className="px-6 py-6">
                <div className="flex flex-wrap justify-center gap-2">
                  {project.tech.map((t) => (
                    <span key={t} className="bg-foreground/5 text-muted-foreground text-[10px] px-2 py-1 rounded-full border border-border whitespace-nowrap">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              <td className="px-6 py-6 text-right">
                <div className="flex items-center justify-end gap-3 text-muted-foreground">
                  ...
                  {project.repoUrl ? (
                    ...
                    <button className="opacity-20 cursor-not-allowed text-muted-foreground">
                      <ExternalLink size={18} />
                    </button>
                  )}
                </div>
